...
export const integrationRoutes = new Elysia({ prefix: "/api/integrations" })
  ...
  .get("/:id", async ({ user, params, set }) => {
    try {
      ...
      console.error("Route error:", err);
      set.status = 500;
      return { error: "Failed to load integration" };
    }
  })
  ...
  .patch("/:id/toggle", async ({ user, params, set }) => {
    try {
      ...
      console.error("Route error:", err);
      set.status = 500;
      return { error: "Failed to toggle integration" };
    }
  })
  ...
  .get(
    ...
    async ({ user, params, query, set }) => {
      try {
        ...
        console.error("Route error:", err);
        set.status = 500;
        return { error: "Failed to load integration events" };
      }
    },
    {
      query: t.Object({
  ...
  .get("/:id/mappings", async ({ user, params, set }) => {
    try {
      ...
      return await db.query.fieldMappings.findMany({
        where: eq(schema.fieldMappings.integrationId, params.id),
      ...
      console.error("Route error:", err);
      set.status = 500;
      return { error: "Failed to load field mappings" };
    }
  })
  ...
  .put(
    ...
    async ({ user, params, body, set }) => {
      try {
        ...
        console.error("Route error:", err);
        set.status = 500;
        return { error: "Failed to update field mappings" };
      }
    },
    {
      body: t.Object({
