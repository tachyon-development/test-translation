"use client";

import { useRef, useEffect, useState, useCallback } from "react";
import {
  renderHistogram,
  generateMockConfidenceData,
  type ConfidenceData,
} from "@/lib/d3/histogram";
import { apiRequest } from "@/lib/api";

export function ConfidenceHistogram() {
  const svgRef = useRef<SVGSVGElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [data, setData] = useState<ConfidenceData | null>(null);
  const [dims, setDims] = useState({ width: 0, height: 0 });

  const fetchData = useCallback(async () => {
    try {
      const res = await apiRequest<{ scores: number[] }>(
        "/api/analytics/ai"
      );
      if (res.scores?.length) {
        setData({ scores: res.scores });
        return;
      }
    } catch {
      // API unavailable
    }
    setData(generateMockConfidenceData());
  }, []);

  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 15_000);
    return () => clearInterval(interval);
  }, [fetchData]);

  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    const obs = new ResizeObserver((entries) => {
      const { width, height } = entries[0].contentRect;
      setDims({ width, height });
    });
    obs.observe(el);
    return () => obs.disconnect();
  }, []);

  useEffect(() => {
    if (!svgRef.current || !data || dims.width === 0) return;
    renderHistogram(svgRef.current, data, dims);
  }, [data, dims]);

  return (
    <div className="relative rounded-xl border border-white/[0.06] bg-[var(--bg-elevated)] p-5">
      <h3 className="mb-3 text-sm font-semibold text-[var(--text-primary)]">
        AI Confidence Distribution
      </h3>
      <div ref={containerRef} className="relative h-56 w-full">
        {!data ? (
          <div className="flex h-full items-center justify-center text-xs text-[var(--text-muted)]">
            Loading confidence data...
          </div>
        ) : (
          <svg ref={svgRef} className="h-full w-full" />
        )}
      </div>
    </div>
  );
}
